...
namespace Mandible.Zlib
{
    ...
    public class InflaterHuffmanTree
	{
		...
		/// Literal length tree
		/// </summary>
		public static InflaterHuffmanTree DefLitLenTree { get; protected set; }

		/// <summary>
		/// Distance tree
		/// </summary>
		public static InflaterHuffmanTree DefDistTree { get; protected set; }
		...
		private void BuildTree(IList<byte> codeLengths)
		{
			...
			*/
			...
			for (int bits = MAX_BITLEN; bits >= 10; bits--)
			{
				...
				{
					_tree[ICSharpCode.SharpZipLib.Zip.Compression.DeflaterHuffman.BitReverse(i)] = (short)((-treePtr << 4) | bits);
					treePtr += 1 << (bits - 9);
				}
			}
			...
			for (int i = 0; i < codeLengths.Count; i++)
			{
				...
				code = nextCode[bits];
				int revcode = ICSharpCode.SharpZipLib.Zip.Compression.DeflaterHuffman.BitReverse(code);
				if (bits <= 9)
				{
					do
					{
